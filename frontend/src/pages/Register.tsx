...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Supabase may require email confirmation. If email confirmation is off,
      // the user is signed in and onAuthStateChange handles the redirect.
    ...
    <div className="min-h-screen bg-sage-50 flex flex-col items-center justify-center px-6 py-12">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-2xl font-bold text-sage-900">Create Account</h1>
          <p className="text-sage-500 text-sm mt-1">Start tracking your products</p>
        </div>

        {error && (
          <div className="p-3 rounded-xl bg-red-50 text-red-600 text-sm font-medium mb-4" data-testid="register-error">
            {error}
          </div>
        )}
        {info && (
          <div className="p-3 rounded-xl bg-sage-100 text-sage-700 text-sm font-medium mb-4" data-testid="register-info">
            {info}
          </div>
        )}
